...
function repository() {
  const calls: unknown[] = [];
  return { calls, findAccountByUserAndEnvironment: async (userId: string, environment: string) => userId === 'teacher-1' && environment === 'SANDBOX' ? { id: 'account-1', environment: 'SANDBOX' } : null, replaceCredential: async (...args: unknown[]) => { calls.push(args); return { id: 'credential-2' }; }, revokeCredential: async (...args: unknown[]) => { calls.push(args); } } as any;
}
