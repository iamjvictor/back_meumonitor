...
export class SubscriptionService {
  ...
  async addMonitor(userId: string, billingSubscriptionId: string, monitorId: string, idempotencyKey?: string) {
    ...
    const activeItems = subscription.items.filter((item: { status: string }) => item.status === 'ACTIVE');
    ...
    try {
      let item = subscription.items.find((candidate: { monitorId: string }) => candidate.monitorId === monitorId);
  ...
  async removeMonitor(userId: string, billingSubscriptionId: string, monitorId: string, idempotencyKey?: string) {
    ...
    if (replay) return replay;
    const item = subscription.items.find((candidate: { monitorId: string; status: string }) => candidate.monitorId === monitorId && candidate.status === 'ACTIVE');
  ...
  async changeInterval(userId: string, billingSubscriptionId: string, interval: BillingInterval, idempotencyKey?: string) {
    ...
    if (replay) return replay;
    const count = subscription.items.filter((item: { status: string }) => item.status === 'ACTIVE').length;
  ...
  private async replay(key: string, subscriptionId: string, studentId: string, operation: string, userId: string, expected: Record<string, unknown> = {}) {
    ...
    const metadata = isRecord(change.metadata) ? change.metadata : {};
    ...
    if (change.subscriptionId !== subscriptionId || change.studentId !== studentId || metadata?.operation !== operation || !sameInput) {
      ...
      throw new AppError({ code: 'IDEMPOTENCY_KEY_REUSED', statusCode: 409, publicMessage: 'A chave de idempotência já foi utilizada com outros dados.' });
    }
    log('monitor.billing_subscription_idempotency_hit', { userId, subscriptionId, operation });
    if (!metadata.result) throw new AppError({ code: 'IDEMPOTENCY_REPLAY_UNAVAILABLE', statusCode: 409, publicMessage: 'Não foi possível repetir esta operação com segurança.' });
